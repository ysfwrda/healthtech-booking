...
The two main architectural approaches considered are Microservices and a Modular Monolith : where the application is a single deployable unit but internally structured into well-defined, loosely coupled modules. where Modular Monoliths are normally the best choice for small teams and at early stages of application development.
...
- More complexity in debugging, especially for requests running through multiple services