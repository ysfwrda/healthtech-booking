...
patients. The specification is ADR-009. It does not ship with this PR — the relay's own
correctness does not depend on it, so it is scoped as the immediate next PR rather than blocking
this one, but it is not optional follow-up: `Notification` has no idempotency protection today,
and this decision raises the probability of the duplicate it's exposed to.
